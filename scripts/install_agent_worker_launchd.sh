...
LABEL="${LABEL:-com.imsg-agent.worker}"
...
usage() {
  cat <<'EOF'
Usage: bash scripts/install_agent_worker_launchd.sh [--no-send] [--print-only]

Installs a user launchd agent that runs:
  uv run imsg-agent-worker

The worker consumes ~/imsg-data/imessage.sqlite, proposes reviewable drafts, and
optionally sends already-approved outbox items. It does not subscribe to imsg rpc.

Environment overrides:
  LABEL=com.imsg-agent.worker
...
STDOUT_LOG="$LOG_DIR/imsg-agent-worker.log"
STDERR_LOG="$LOG_DIR/imsg-agent-worker.err.log"
UID_VALUE="$(id -u)"
...
<dict>
  ...
  <array>
    ...
    <string>run</string>
    <string>imsg-agent-worker</string>
EOF
