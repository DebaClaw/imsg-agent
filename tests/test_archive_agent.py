...
class RateLimitedDraftingClient:
    async def create_draft(
        self,
        *,
        model: str,
...
@pytest.mark.asyncio
async def test_archive_agent_worker_leaves_cursor_on_rate_limit(tmp_path: Path) -> None:
    archive = IMessageArchive(tmp_path / "imessage.sqlite")
    archive.upsert_chat(_chat())
    archive.upsert_message(_message(rowid=100))
    store = MessageStore(tmp_path / "data")
    ...
    drafter = Drafter(store, RateLimitedDraftingClient(), now=NOW)
    worker = ArchiveAgentWorker(
        archive=archive,
        store=store,
        drafter=drafter,
