...
│   ├── web_app.py         ← local-only web GUI/API over archive + approval artifacts
...
**Current (Phase 6+):** A local web UI may live in this repo as an operator surface over
the same archive and Markdown approval artifacts. The UI must read/write the same
frontmatter fields and must keep the file-based approval path working always.
...
- Do NOT bypass the Markdown approval/outbox/sent/error artifacts from UI code
